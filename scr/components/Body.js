...
const Body = () => {
    ...
    const fetchData = async () =>{
        ...
        setListOfRestorents(json?.data?.cards[4]?.card?.card?.gridElements?.infoWithStyle?.restaurants);
        setFilteredRestaunrents(json?.data?.cards[4]?.card?.card?.gridElements?.infoWithStyle?.restaurants);
    ...
    return (listOfRestorents.length === 0) ? (<Shimmer />) :
        (<>
        <div className="filter-btn">
            ...
            <button className="search-btn" onClick={()=>{
                const filteredRes = listOfRestorents.filter((res)=> res.info.name.toLowerCase().includes(searchText.toLowerCase()))
